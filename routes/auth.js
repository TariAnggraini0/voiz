  ...
  async (req, res) => {
    ...
    try {
      ...
      const [result] = await pool.query(
        `INSERT INTO users (nama, email, password_hash, nim, prodi)
         VALUES (?, ?, ?, ?, ?)`,
        [nama, email, hash, nim || null, prodi || null]
      );

      const user = { id: result.insertId, email, role: 'mahasiswa', nama };
      ...
      return res.status(201).json({
        ...
        token,
        user: { id: user.id, nama, email, role: 'mahasiswa', nim, prodi },
      });
    } catch (err) {
  ...
  async (req, res) => {
    ...
    try {
      const [rows] = await pool.query(
        'SELECT id, nama, email, password_hash, role, nim, prodi, is_active FROM users WHERE email = ?',
        [email]
      );
      ...
      return res.json({
        ...
        user: {
          ...
          role: user.role, nim: user.nim, prodi: user.prodi,
        },
      });
    } catch (err) {
...
router.get('/me', verifyToken, async (req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT id, nama, email, role, nim, prodi, avatar_url, created_at FROM users WHERE id = ?',
      [req.user.id]
    );
    if (rows.length === 0) {
