...
// ─────────────────────────────────────────────────────────────
//  PATCH  /api/admin/aspirasi/:id/status  — ubah status
//  GET    /api/admin/stats                — dashboard statistik
//  GET    /api/admin/aspirasi             — semua aspirasi (detail)
//  GET    /api/admin/notifikasi           — notif admin
...
// ── PATCH status aspirasi ─────────────────────────────────
router.patch(
  '/aspirasi/:id/status',
  [...guard, body('status').isIn(['diterima','diproses','selesai','ditolak'])],
  async (req, res) => {
    ...
    try {
      ...
      const [[asp]] = await pool.query(
        'SELECT status, user_id, judul FROM aspirasi WHERE id = ?', [id]
      );
      if (!asp) {
      ...
      if (prioritas) { updateFields.push('prioritas = ?'); updateParams.push(prioritas); }
      updateParams.push(id);

      await pool.query(
        `UPDATE aspirasi SET ${updateFields.join(', ')} WHERE id = ?`,
        updateParams
      );

      // Catat log
      await pool.query(
        `INSERT INTO status_log (aspirasi_id, admin_id, status_lama, status_baru, catatan)
         VALUES (?, ?, ?, ?, ?)`,
        [id, req.user.id, asp.status, status, catatan || null]
      );

      // Kirim notifikasi ke pengirim (kalau bukan anonim)
      if (asp.user_id) {
        const labelStatus = { diterima:'Diterima', diproses:'Sedang Diproses', selesai:'Selesai', ditolak:'Ditolak' };
        await pool.query(
          `INSERT INTO notifikasi (user_id, aspirasi_id, judul, pesan)
           VALUES (?, ?, ?, ?)`,
          [
            asp.user_id, id,
            `Aspirasi ${labelStatus[status]}`,
            `Aspirasi "${asp.judul}" kini berstatus: ${labelStatus[status]}.${catatan ? ' Catatan: ' + catatan : ''}`,
          ]
        );
      }
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT
       ...
       WHERE ${whereSQL}
       ORDER BY
         FIELD(a.status,'menunggu','diterima','diproses','selesai','ditolak'),
    ...
    const [[{ total }]] = await pool.query(
      `SELECT COUNT(*) AS total FROM aspirasi a WHERE ${whereSQL}`, params
    );
...
router.get('/notifikasi', verifyToken, async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT id, aspirasi_id, judul, pesan, is_read, created_at
