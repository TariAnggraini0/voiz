...
// ─────────────────────────────────────────────────────────────
//  GET    /api/aspirasi          — list (publik, dengan filter)
//  POST   /api/aspirasi          — kirim aspirasi (auth)
//  GET    /api/aspirasi/:id      — detail
//  GET    /api/aspirasi/saya     — aspirasi milik sendiri (auth)
...
// ── GET /aspirasi — list dengan filter & pagination ────────
router.get(
  '/',
  [
    query('page').optional().isInt({ min: 1 }),
    query('limit').optional().isInt({ min: 1, max: 50 }),
    query('kategori_id').optional().isInt(),
    query('status').optional().isIn(['menunggu','diterima','diproses','selesai','ditolak']),
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (kategoriId) { where.push('a.kategori_id = ?'); params.push(kategoriId); }
      if (status)     { where.push('a.status = ?');      params.push(status); }
      if (q) {
        where.push('MATCH(a.judul, a.isi) AGAINST(? IN BOOLEAN MODE)');
        params.push(`${q}*`);
      }
      ...
      const [[{ total }]] = await pool.query(
        `SELECT COUNT(*) AS total FROM aspirasi a WHERE ${whereSQL}`,
        params
      );
...
// ── GET /aspirasi/saya — aspirasi milik user login ─────────
router.get('/saya', verifyToken, async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT
         a.id, a.judul, a.status, a.prioritas, a.vote_count,
         a.is_anonim, a.created_at,
         k.nama AS kategori_nama, k.icon AS kategori_icon, k.warna AS kategori_warna
       FROM aspirasi a
       JOIN kategori k ON k.id = a.kategori_id
       WHERE a.user_id = ?
  ...
  try {
    ...
    const [logs] = await pool.query(
      `SELECT sl.status_lama, sl.status_baru, sl.catatan, sl.created_at,
              IF(u.id IS NULL, 'Sistem', u.nama) AS oleh
       FROM status_log sl
...
// ── POST /aspirasi — kirim aspirasi baru ──────────────────
router.post(
  '/',
  verifyToken,
  [
    body('judul').trim().notEmpty().isLength({ max: 200 }).withMessage('Judul wajib diisi (maks 200 karakter).'),
    body('isi').trim().notEmpty().withMessage('Isi aspirasi wajib diisi.'),
    body('kategori_id').isInt({ min: 1 }).withMessage('Kategori tidak valid.'),
  ...
  async (req, res) => {
    ...
    try {
      const [result] = await pool.query(
        `INSERT INTO aspirasi (user_id, kategori_id, judul, isi, is_anonim)
         VALUES (?, ?, ?, ?, ?)`,
      ...
      // Log status awal
      await pool.query(
        `INSERT INTO status_log (aspirasi_id, status_baru, catatan)
         VALUES (?, 'menunggu', 'Aspirasi dikirim.')`,
        [result.insertId]
      );
      ...
      if (admins.length > 0) {
        const notifValues = admins.map(a => [
          ...
          'Aspirasi Baru Masuk',
          `"${judul}" telah dikirim dan menunggu peninjauan.`,
        ]);
        await pool.query(
